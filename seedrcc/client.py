...
class Seedr:
    ...
    def from_login(
        ...
        **httpx_kwargs: Any,
        ...
        Returns:
            An initialized `Seedr` client instance using cookie-based authentication.

        Example:
            ```python
            # Cookie-only auth
            client = Seedr.from_login("email", "password")

            # Combined cookie + OAuth auth
            client = Seedr.from_login("email", "password", token=Token(access_token="..."))
            ```
        """
        httpx_kwargs.setdefault("timeout", timeout)
        httpx_kwargs.setdefault("proxy", proxy)
    ...
    def add_torrent(
        ...
        Example:
            ...
            result = client.add_torrent(torrent_file="https://...", force_upload=True)
            ```
        """
    ...
    def _cookie_api_request(
        ...
        response = self._make_http_request(self._client, method, url, **request_kwargs)
